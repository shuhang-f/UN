...
type View = "desk" | "portfolio" | "saved" | "preferences";
...
const statusLabel: Record<UnReviewCase["status"], string> = { "draft-ready": "Draft prepared", "needs-evidence": "Needs evidence", blocked: "Increase on hold", upcoming: "Upcoming" };
...
function UNDesk({ onRestartDemo, demoEmail }: { onRestartDemo: () => void; demoEmail: string }) {
  ...
  function runReview(next = preferences) {
    try {
      ...
      setTab("overview"); setFilter("all"); setView("desk");
      const count = result.filter(item => item.status !== "upcoming").length;
      setMessage(`Review complete. ${count} ${count === 1 ? "unit needs" : "units need"} attention as of ${date(dateInput)}.`);
  ...
  async function saveReview() {
    ...
    try {
      ...
      setSavedSelection(data.record.id); setReviewing(false); setMessage(`Unit ${selected.unit.unitNumber} review saved in the local demo database.`);
  ...
  return <div className="un-app">
    <aside className="un-sidebar">
      ...
      <nav aria-label="Workspace">{([
        ["desk", "grid", "Review desk"], ["portfolio", "building", "Portfolio"], ["saved", "folder", "Saved reviews"], ["preferences", "sliders", "Preferences"],
    ...
    <div className="un-main">
      ...
      <main className="un-content">
        ...
        {view === "desk" && <>
          <div className="un-stats"><div><span>Ready for your attention</span><strong>{due.length}<small>units in review</small></strong><p>Within your {preferences.reviewLeadDays}-day planning window</p></div><div><span>One step closer</span><strong>{drafted.length}<small>draft{drafted.length === 1 ? "" : "s"} prepared</small></strong><p>Prepared for an internal review</p></div><div><span>A clearer next step</span><strong>{held.length}<small>need a closer look</small></strong><p>Evidence or timing to resolve</p></div></div>
          <div className="un-section-title"><div><h2>Today’s review</h2><span>Simulated · {date(asOf)}</span></div><button className="un-text-button" onClick={() => { setForm(preferences); setView("preferences"); }}><Icon name="sliders" size={15}/>{preferences.reviewLeadDays}-day window</button></div>
          <div className="un-review-grid">
            <section className="un-queue" aria-label="Unit review queue"><div className="un-filters" aria-label="Filter reviews"><button className={filter === "all" ? "selected" : ""} onClick={() => changeFilter("all")}>Needs attention <span>{due.length}</span></button><button className={filter === "upcoming" ? "selected" : ""} onClick={() => changeFilter("upcoming")}>Upcoming</button></div>
              ...
              {visibleCases.map(item => <button key={item.id} className={`un-unit-card ${selectedId === item.unit.id ? "selected" : ""}`} onClick={() => selectUnit(item.unit.id)} aria-pressed={selectedId === item.unit.id}><div className="un-card-top"><span className="un-unit-number">{item.unit.unitNumber}</span><Status status={item.status}/></div><h3>{item.unit.propertyName}</h3><p>{item.status === "draft-ready" ? "Renewal approaching. Your rent proposal is ready to review." : item.blockers[0] || item.summary}</p><div className="un-card-bottom"><span className={item.daysUntilRenewal >= 0 && item.daysUntilRenewal <= 30 ? "un-renewal-soon" : undefined}><Icon name="clock" size={13}/>{item.daysUntilRenewal >= 0 ? `${item.daysUntilRenewal} days to lease end` : "Lease end passed"}</span><Icon name="arrow" size={16}/></div></button>)}
            ...
            <section ref={detailRef} className="un-detail" aria-label={`Unit ${selected.unit.unitNumber} review`}>
              ...
              <div className="un-detail-tabs" role="tablist" aria-label="Review details">{(["overview", "evidence", "draft"] as DetailTab[]).map(id => <button key={id} role="tab" aria-selected={tab === id} aria-controls={`un-panel-${id}`} id={`un-tab-${id}`} onClick={() => setTab(id)}>{{ overview: "Review overview", evidence: `Evidence · ${selected.evidence.length}`, draft: "Notice draft" }[id]}{id === "draft" && selected.draft && <span className="un-tab-dot"/>}</button>)}</div>
              <div className="un-detail-body" role="tabpanel" id={`un-panel-${tab}`} aria-labelledby={`un-tab-${tab}`}>
                {tab === "overview" && <>
                  <div className={`un-summary-box ${selected.status}`}><Icon name={selected.draft ? "spark" : "clock"} size={20}/><div><strong>{statusLabel[selected.status]}</strong><p>{selected.summary}</p></div></div>
                  ...
                  <h3 className="un-subheading">The next steps</h3><div className="un-steps">{selected.tasks.map((task, index) => <div key={index}><span>{String(index + 1).padStart(2, "0")}</span><div><strong>{task.title}</strong><p>{task.details}</p><small>{task.owner}</small></div></div>)}</div>
                ...
                {tab === "draft" && (selected.draft ? <><div className="un-draft-note"><Icon name="file" size={18}/><div><strong>Internal draft · pending review</strong><p>Illustrative demo template. Verify required wording and service requirements before using a real notice.</p></div></div><article className="un-paper"><div className="un-paper-logo">UN<span> / JUNIPER RESIDENTIAL</span></div><p className="un-paper-label">FICTIONAL DEMONSTRATION</p><h3>{selected.draft.title}</h3><pre>{selected.draft.body}</pre><footer>Template {selected.draft.templateVersion}</footer></article></> : <div className="un-empty un-no-draft"><Icon name="file" size={34}/><h3>{selected.status === "upcoming" ? "We’ll start with the review window." : "A clear record comes first."}</h3><p>{selected.status === "upcoming" ? "This lease is outside the active planning window. Update your preferences or advance the demo date to review it." : "Resolve the listed evidence or timing issues before preparing a definitive rent-increase draft."}</p><button className="un-secondary" onClick={() => setTab("overview")}>View next steps <Icon name="arrow" size={15}/></button></div>)}
              </div>
              <footer className="un-detail-footer"><span><Icon name="folder" size={15}/>Saved locally after your review</span><button className="un-primary" disabled={!ready} onClick={() => { setReviewing(true); setError(""); }}>{selected.draft ? "Review & save draft" : "Review & save follow-up"}<Icon name="arrow" size={16}/></button></footer>
            </section>
          </div>
    ...
    {reviewing && <dialog ref={dialogRef} className="un-modal" aria-labelledby="un-review-title" onCancel={e => { e.preventDefault(); if (!saving) setReviewing(false); }}><div className="un-modal-top"><span className="un-eyebrow">YOUR REVIEW, ON RECORD</span><button className="un-quiet" aria-label="Close review" disabled={saving} onClick={() => setReviewing(false)}><Icon name="close"/></button></div><h2 id="un-review-title">Save Unit {selected.unit.unitNumber}’s review</h2><p>Keep the full evidence snapshot, rent calculation, {selected.draft ? "internal draft" : "open questions"}, and next steps.</p><div className="un-modal-summary"><Status status={selected.status}/><span>{preferences.reviewer}</span></div><label>Reviewer note<textarea autoFocus disabled={saving} maxLength={4000} rows={4} placeholder="Add your decision, a correction to investigate, or context for the next person…" value={note} onChange={e => setNote(e.target.value)}/></label><p className="un-modal-caption">This saves an internal review in the local demo database. It does not serve a notice or update anyone’s rent.</p>{error && <p role="alert" className="un-inline-error">{error}</p>}<div className="un-modal-actions"><button className="un-secondary" disabled={saving} onClick={() => setReviewing(false)}>Cancel</button><button className="un-primary" disabled={saving || !ready} onClick={() => void saveReview()}>{saving ? "Saving review…" : "Confirm review & save"}<Icon name="check" size={16}/></button></div></dialog>}
    {assistantOpen && modelConfigured && <aside className="un-assistant-panel"><div><strong>Ask UN</strong><button className="un-quiet" aria-label="Close assistant" onClick={() => setAssistantOpen(false)}><Icon name="close"/></button></div><p>Discuss the fictional records on this desk with your configured assistant.</p><UNAssistant cases={cases} selectedId={selectedId} asOf={asOf} onSelect={selectUnit}/></aside>}
  </div>;
}
